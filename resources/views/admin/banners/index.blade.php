...
@section('content')
<h1 class="text-2xl font-semibold mb-6 text-[#f5f5f5]">HOMEPAGE BANNER</h1>
    ...
    <table class="table-fixed w-full border-collapse" id="banners-table">
        ...
        <tbody>
            ...
            <tr class="border-t hover:bg-gray-50 transition align-middle">
                ...
                <td class="px-10 py-3 text-[#0a1a3a] font-semibold text-center">
                    {{ $banner->title }}
                </td>
                <!-- <td class="px-10 py-3 text-gray-700">{!! $banner->subtitle !!}</td> -->
                <td class="px-10 py-3 text-center align-middle">
                    <div class="flex justify-center">
                        <img src="{{ asset($banner->image) }}"
                            class="w-24 h-14 object-cover rounded shadow-md"
                            alt="Banner">
                    </div>
                </td>
                <td class="px-6 py-3 whitespace-nowrap align-middle">
                    <div class="flex justify-center items-center gap-2">
                        <a href="{{ route('admin.banners.edit', $banner->id) }}"
                            class="px-3 py-1 rounded text-white bg-[#25464b] hover:bg-[#0c2627] transition text-xs font-semibold">
                            Edit
                        </a>
                        <form action="{{ route('admin.banners.destroy', $banner->id) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="button"
                                class="delete-btn px-3 py-1 rounded text-white bg-red-500 hover:bg-red-600 transition text-xs font-semibold">
                                Delete
                            </button>
                        </form>
                    </div>
