...
<div class="max-w-screen-xl mx-auto bg-white px-14 py-12 rounded-2xl shadow-xl border border-gray-200">
    ...
    <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        {{-- Subtitle (optional – uncomment if you use it) --}}
        {{--
        <div>
...
<div
    ...
    <p class="text-sm text-gray-500 mb-5">
        Upload a new banner image (Max 5MB • JPG • PNG • WEBP • GIF)<br>
        <span class="text-xs text-gray-400">Leave this empty if you don’t want to change the image.</span>
    </p>
    ...
    <div class="mb-6 flex justify-center">
        <div class="inline-block text-center">
            ...
            <img src="{{ asset($banner->image) }}"
                alt="Current Banner"
                class="w-80 h-48 object-cover rounded-lg shadow border border-gray-300 mx-auto">
        </div>
    </div>
    @endif
...
{{-- New Image Preview Section (only when choosing a new file) --}}
<div id="preview-container" class="hidden mt-6 text-center">
    <p class="font-semibold text-[#25464b] mb-3">New Image Preview</p>
...
<script>
    // Only init CKEditor if #subtitle exists (since it's commented out by default)
    ...
    // Image Preview for NEW upload (doesn't overwrite the existing image until submit)
